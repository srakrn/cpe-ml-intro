	...
	\begin{itemize}
		\item<2-> A result of the combination between...
		      \begin{itemize}
			      ...
			      \item<4-> \textbf{sample data} for such the method
		      \end{itemize}
	\end{itemize}
	\begin{columns}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item<7-> $l_\infty$ Norm: $\left|x\right|_\infty = \texttt{max}(x_1, x_2, ..., x_n)$ (Maximum norm)
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{itemize}
			\item<3-> $P(\textrm{Male}|\textrm{Long hair}) = \frac{1}{3}$
			\item<4-> $P(\textrm{Female}|\textrm{Long hair}) = \frac{2}{3}$
		\end{itemize}
		\onslide<5-> Therefore, we guess that the long-haired person is more likely to be a female.
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{block}{Problem}<4->
			When the list of the bulb that went on was given, can we determine the number?
		\end{block}
	\end{columns}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{columns}
		...
		\column{0.7\textwidth}
		\begin{block}{Problem}
			When the image of the handwriting was given, can we determine the number?
		\end{block}
		...
		\item<3-> Testing: \textit{*thinking*}
			\begin{itemize}
				...
				\item<5-> = 600,000,000 calculations to be made\\
					(this excludes sorting, of which is a $\mathcal{O}(n \log n)$ process)
...
\begin{frame}
	...
	\begin{columns}[t]
		...
		\onslide<4-> Why can't XOR gate be created using a perceptron?
	\end{columns}
\end{frame}
